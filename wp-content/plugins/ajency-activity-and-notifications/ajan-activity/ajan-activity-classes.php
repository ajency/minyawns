...
class AJAN_Activity_Activity {
	...
	public function populate() {
		...
		$row = wp_cache_get( $this->id, 'ajan_activity' );

		if ( false === $row ) {
			$row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$ajan->activity->table_name} WHERE id = %d", $this->id ) );

			wp_cache_set( $this->id, $row, 'ajan_activity' );
		}
		...
		$action = ajan_activity_generate_action_string( $this );
		if ( false !== $action ) {
			$this->action = $action;
	...
	public function save() {
		...
		$this->user_id           = apply_filters_ref_array( 'ajan_activity_user_id_before_save',           array( $this->user_id,           &$this ) );
		$this->component         = apply_filters_ref_array( 'ajan_activity_component_before_save',         array( $this->component,         &$this ) );
		...
		do_action_ref_array( 'ajan_activity_before_save', array( &$this ) );
		...
		if ( $this->id )
			...
			$q = $wpdb->prepare( "INSERT INTO {$ajan->activity->table_name} ( user_id, component, type, action, content,   date_recorded, item_id, secondary_item_id, hide_sitewide ) VALUES ( %d, %s, %s, %s, %s,  %s, %d, %d, %d )", $this->user_id, $this->component, $this->type, $this->action, $this->content,  $this->date_recorded, $this->item_id, $this->secondary_item_id, $this->hide_sitewide );
		...
		// If an existing activity item, prevent any changes to the content generating new @mention notifications.
			...
			add_filter( 'ajan_activity_at_name_do_notifications', '__return_false' );

		do_action_ref_array( 'ajan_activity_after_save', array( &$this ) );
	 ...
	 *
	 * @see AJAN_Activity_Activity::get_filter_sql() for a description of the
	 *      'filter' parameter.
	 ...
	 *     @type array $filter See AJAN_Activity_Activity::get_filter_sql().
	...
	public static function get( $args = array() ) {
		...
		if ( !is_array( $args ) || func_num_args() > 1 ) {
			_deprecated_argument( __METHOD__, '1.6', sprintf( __( 'Arguments passed to %1$s should be in an associative array. See the inline documentation at %2$s for more details.', 'ajency-activity-and-notifications' ), __METHOD__, __FILE__ ) );
			...
			$args      = ajan_core_parse_args_array( $old_args_keys, $func_args );
		}
		...
		$from_sql   = " FROM {$ajan->activity->table_name} a";
		...
		// Filtering
		if ( $filter && $filter_sql = AJAN_Activity_Activity::get_filter_sql( $filter ) )
		...
		$where_conditions = apply_filters( 'ajan_activity_get_where_conditions', $where_conditions, $r, $select_sql, $from_sql, $join_sql );

		// Join the where conditions together
		$where_sql = 'WHERE ' . join( ' AND ', $where_conditions );

		// Define the preferred order for indexes
		$indexes = apply_filters( 'ajan_activity_preferred_index_order', array( 'user_id', 'item_id', 'secondary_item_id', 'date_recorded', 'component', 'type', 'hide_sitewide', 'is_spam' ) );
		...
		if ( apply_filters( 'ajan_use_legacy_activity_query', false, __METHOD__, $r ) ) {
			...
			$from_sql   = " FROM {$ajan->activity->table_name} a LEFT JOIN {$wpdb->users} u ON a.user_id = u.ID";

			if ( ! empty( $page ) && ! empty( $per_page ) ) {
				$pag_sql    = $wpdb->prepare( "LIMIT %d, %d", absint( ( $page - 1 ) * $per_page ), $per_page );
				$activities = $wpdb->get_results( apply_filters( 'ajan_activity_get_user_join_filter', "{$select_sql} {$from_sql} {$join_sql} {$where_sql} ORDER BY a.date_recorded {$sort} {$pag_sql}", $select_sql, $from_sql, $where_sql, $sort, $pag_sql ) );
			} else {
				$activities = $wpdb->get_results( apply_filters( 'ajan_activity_get_user_join_filter', "{$select_sql} {$from_sql} {$join_sql} {$where_sql} ORDER BY a.date_recorded {$sort}", $select_sql, $from_sql, $where_sql, $sort ) );
			}
			...
			$activity_ids_sql = apply_filters( 'ajan_activity_paged_activities_sql', $activity_ids_sql, $r );

			$activity_ids = $wpdb->get_col( $activity_ids_sql );
			$activities   = self::get_activity_data( $activity_ids );
		}

		$total_activities_sql = apply_filters( 'ajan_activity_total_activities_sql', "SELECT count(DISTINCT a.id) FROM {$ajan->activity->table_name} a {$join_sql} {$where_sql}", $where_sql, $sort );
		...
		$activities = AJAN_Activity_Activity::generate_action_strings( $activities );
	...
	protected static function get_activity_data( $activity_ids = array() ) {
		...
		$activities   = array();
		$uncached_ids = ajan_get_non_cached_ids( $activity_ids, 'ajan_activity' );
		...
		if ( ! empty( $uncached_ids ) ) {
			...
			$queried_adata = $wpdb->get_results( "SELECT * FROM {$ajan->activity->table_name} WHERE id IN ({$uncached_ids_sql})");

			// Put that data into the placeholders created earlier,
			// and add it to the cache
			foreach ( (array) $queried_adata as $adata ) {
				wp_cache_set( $adata->id, $adata, 'ajan_activity' );
			}
		}

		// Now fetch data from the cache
		foreach ( $activity_ids as $activity_id ) {
			$activities[] = wp_cache_get( $activity_id, 'ajan_activity' );
		}

		// Then fetch user data
		$user_query = new AJAN_User_Query( array(
	...
	protected static function append_user_fullnames( $activities ) {
		...
		if ( ajan_is_active( 'xprofile' ) && ! empty( $activities ) ) {
			$activity_user_ids = wp_list_pluck( $activities, 'user_id' );

			if ( ! empty( $activity_user_ids ) ) {
				$fullnames = ajan_core_get_user_displaynames( $activity_user_ids );
				if ( ! empty( $fullnames ) ) {
	 ...
	 * info is fetched separately in AJAN_Activity_Activity::get_activity_data().
	...
	protected static function prefetch_object_data( $activities ) {
		return apply_filters( 'ajan_activity_prefetch_object_data', $activities );
	}

	/**
	 * Generate action strings for the activities located in AJAN_Activity_Activity::get().
	 *
	...
	protected static function generate_action_strings( $activities ) {
		foreach ( $activities as $key => $activity ) {
			$generated_action = ajan_activity_generate_action_string( $activity );
	...
	/**
	 * Get the SQL for the 'meta_query' param in AJAN_Activity_Activity::get().
	 *
	 * We use WP_Meta_Query to do the heavy lifting of parsing the
	 * meta_query array and creating the necessary SQL clauses. However,
	 * since AJAN_Activity_Activity::get() builds its SQL differently than
	...
	public static function get_meta_query_sql( $meta_query = array() ) {
		...
		if ( ! empty( $meta_query ) ) {
			...
			// $wpdb->activitymeta
			$wpdb->activitymeta = activitynotifications()->activity->table_name_meta;
	 ...
	 *
	 * As of 1.5.x, use AJAN_Activity_Activity::get() with an 'in' parameter instead.
	 *
	 * @since ajency-activity-and-notifications (1.2)
	 *
	 * @deprecated 1.5
	 * @deprecated Use AJAN_Activity_Activity::get() with an 'in' parameter instead.
	 *
	...
	public static function get_specific( $activity_ids, $max = false, $page = 1, $per_page = 25, $sort = 'DESC', $display_comments = false ) {
		_deprecated_function( __FUNCTION__, '1.5', 'Use AJAN_Activity_Activity::get() with the "in" parameter instead.' );
		return AJAN_Activity_Activity::get( $max, $page, $per_page, $sort, false, false, $display_comments, false, false, $activity_ids );
	}
	...
	public static function get_id( $user_id, $component, $type, $item_id, $secondary_item_id, $action, $content, $date_recorded ) {
		...
		return $wpdb->get_var( "SELECT id FROM {$ajan->activity->table_name} {$where_sql}" );
	}
	...
	public static function delete( $args = array() ) {
		...
		$activity_ids = $wpdb->get_col( "SELECT id FROM {$ajan->activity->table_name} {$where_sql}" );

		if ( ! $wpdb->query( "DELETE FROM {$ajan->activity->table_name} {$where_sql}" ) ) {
			return false;
		}
		...
		if ( $activity_ids ) {
			...
			$activity_comment_ids = $wpdb->get_col( "SELECT id FROM {$ajan->activity->table_name} {$activity_comments_where_sql}" );

			// We have activity comments!
			if ( ! empty( $activity_comment_ids ) ) {
				// Delete activity comments
				$wpdb->query( "DELETE FROM {$ajan->activity->table_name} {$activity_comments_where_sql}" );
			...
			AJAN_Activity_Activity::delete_activity_meta_entries( $activity_ids );
		}
	...
	public static function delete_activity_item_comments( $activity_ids = array(), $delete_meta = true ) {
		...
		if ( $delete_meta ) {
			// Fetch the activity comment IDs for our deleted activity items
			$activity_comment_ids = $wpdb->get_col( "SELECT id FROM {$ajan->activity->table_name} WHERE type = 'activity_comment' AND item_id IN ({$activity_ids})" );
		...
		return $wpdb->query( "DELETE FROM {$ajan->activity->table_name} WHERE type = 'activity_comment' AND item_id IN ({$activity_ids})" );
	}
	...
	public static function delete_activity_meta_entries( $activity_ids = array() ) {
		...
		foreach ( $activity_ids as $activity_id ) {
			ajan_activity_delete_meta( $activity_id );
		}
	 ...
	 * @since ajency-activity-and-notifications (1.2)
	 *
	 * @global BuddyPress $ajan The one true BuddyPress instance.
	 * @global wpdb $wpdb WordPress database object.
	 *
	...
	public static function get_activity_comments( $activity_id, $left, $right,  $top_level_parent_id = 0 ) {
		...
		$comments = wp_cache_get( $activity_id, 'ajan_activity_comments' );
		...
		// A true cache miss
			...
			if ( ajan_is_active( 'xprofile' ) ) {
				...
				$fullname_from = ", {$ajan->profile->table_name_data} pd ";
			...
			$func_args = func_get_args();
			if ( apply_filters( 'ajan_use_legacy_activity_query', false, __METHOD__, $func_args ) ) {
				$sql = apply_filters( 'ajan_activity_comments_user_join_filter', $wpdb->prepare( "SELECT a.*, u.user_email, u.user_nicename, u.user_login, u.display_name{$fullname_select} FROM {$ajan->activity->table_name} a, {$wpdb->users} u{$fullname_from} WHERE u.ID = a.user_id {$fullname_where} AND a.type = 'activity_comment' {$spam_sql} AND a.item_id = %d AND a.mptt_left > %d AND a.mptt_left < %d ORDER BY a.date_recorded ASC", $top_level_parent_id, $left, $right ), $activity_id, $left, $right, $spam_sql );
			...
			foreach ( $ref as &$r ) {
				...
				while ( $parent_id !== $r->item_id ) {
					...
					if ( empty( $ref[ $parent_id ] ) ) {
						$direct_parent = new AJAN_Activity_Activity( $parent_id );
			...
			wp_cache_set( $activity_id, $cache_value, 'ajan_activity_comments' );
		}
	...
	public static function rebuild_activity_comment_tree( $parent_id, $left = 1 ) {
		...
		$descendants = AJAN_Activity_Activity::get_child_comments( $parent_id );

		// Loop the descendants and recalculate the left and right values
		foreach ( (array) $descendants as $descendant )
			$right = AJAN_Activity_Activity::rebuild_activity_comment_tree( $descendant->id, $right );

		// We've got the left value, and now that we've processed the children
		// of this node we also know the right value
		if ( 1 == $left )
			$wpdb->query( $wpdb->prepare( "UPDATE {$ajan->activity->table_name} SET mptt_left = %d, mptt_right = %d WHERE id = %d", $left, $right, $parent_id ) );
		else
			$wpdb->query( $wpdb->prepare( "UPDATE {$ajan->activity->table_name} SET mptt_left = %d, mptt_right = %d WHERE type = 'activity_comment' AND id = %d", $left, $right, $parent_id ) );
	...
	public static function get_child_comments( $parent_id ) {
		...
		return $wpdb->get_results( $wpdb->prepare( "SELECT id FROM {$ajan->activity->table_name} WHERE type = 'activity_comment' AND secondary_item_id = %d", $parent_id ) );
	}
	...
	public static function get_recorded_components( $skip_last_activity = true ) {
		...
		if ( $skip_last_activity ) {
			$components = $wpdb->get_col( "SELECT DISTINCT component FROM {$ajan->activity->table_name} WHERE action != '' AND action != 'last_activity' ORDER BY component ASC" );
		} else {
			$components = $wpdb->get_col( "SELECT DISTINCT component FROM {$ajan->activity->table_name} ORDER BY component ASC" );
		}
	...
	public static function get_sitewide_items_for_feed( $limit = 35 ) {
		$activities    = ajan_activity_get_sitewide( array( 'max' => $limit ) );
		$activity_feed = array();
	 ...
	 *
	 * @see AJAN_Activity_Activity::get_filter_sql()
	 *
	 ...
	 *
	 * @since ajency-activity-and-notifications (1.5.0)
	 *
	...
	public static function get_filter_sql( $filter_array ) {
		...
		if ( !empty( $filter_array['user_id'] ) ) {
			$user_sql = AJAN_Activity_Activity::get_in_operator_sql( 'a.user_id', $filter_array['user_id'] );
			if ( !empty( $user_sql ) )
				$filter_sql[] = $user_sql;
		}

		if ( !empty( $filter_array['object'] ) ) {
			$object_sql = AJAN_Activity_Activity::get_in_operator_sql( 'a.component', $filter_array['object'] );
			if ( !empty( $object_sql ) )
				$filter_sql[] = $object_sql;
		}

		if ( !empty( $filter_array['action'] ) ) {
			$action_sql = AJAN_Activity_Activity::get_in_operator_sql( 'a.type', $filter_array['action'] );
			if ( ! empty( $action_sql ) )
				$filter_sql[] = $action_sql;
		}

		if ( !empty( $filter_array['primary_id'] ) ) {
			$pid_sql = AJAN_Activity_Activity::get_in_operator_sql( 'a.item_id', $filter_array['primary_id'] );
			if ( !empty( $pid_sql ) )
				$filter_sql[] = $pid_sql;
		}

		if ( !empty( $filter_array['secondary_id'] ) ) {
			$sid_sql = AJAN_Activity_Activity::get_in_operator_sql( 'a.secondary_item_id', $filter_array['secondary_id'] );
			if ( !empty( $sid_sql ) )
				$filter_sql[] = $sid_sql;
		}
	...
	public static function get_last_updated() {
		...
		return $wpdb->get_var( "SELECT date_recorded FROM {$ajan->activity->table_name} ORDER BY date_recorded DESC LIMIT 1" );
	}
	...
	public static function total_favorite_count( $user_id ) {
		if ( !$favorite_activity_entries = ajan_get_user_meta( $user_id, 'ajan_favorite_activities', true ) )
			return 0;
	...
	public static function check_exists_by_content( $content ) {
		...
		return $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$ajan->activity->table_name} WHERE content = %s", $content ) );
	}
	...
	public static function hide_all_for_user( $user_id ) {
		...
		return $wpdb->get_var( $wpdb->prepare( "UPDATE {$ajan->activity->table_name} SET hide_sitewide = 1 WHERE user_id = %d", $user_id ) );
	}
}
 ...
 *
 * See {@link ajan_activity_action_sitewide_feed()} as an example.
 *
 * Accepted parameters:
 ...
 *   activity_args    - Arguments passed to {@link ajan_has_activities()}
 *
 * @since ajency-activity-and-notifications (1.8)
 */
class AJAN_Activity_Feed {
	/**
	 ...
	 *
	 * @see AJAN_Feed::__construct() This is where $data is added
	 * @var array
	 */
	protected $data;
	...
	public function __construct( $args = array() ) {
		...
		if ( false === (bool) apply_filters( 'ajan_activity_enable_feeds', true ) ) {
			global $wp_query;
		...
		$this->data = wp_parse_args( $args, array(
			...
			// Activity arguments passed to ajan_has_activities()
			'activity_args'    => array()
		) );

		// Plugins can use this filter to modify the feed before it is setup
		do_action_ref_array( 'ajan_activity_feed_prefetch', array( &$this ) );
		...
		if ( empty( $this->id ) ) {
			_doing_it_wrong( 'AJAN_Activity_Feed', __( "RSS feed 'id' must be defined", 'ajency-activity-and-notifications' ), 'BP 1.8' );
			return false;
		}

		// Plugins can use this filter to modify the feed after it's setup
		do_action_ref_array( 'ajan_activity_feed_postfetch', array( &$this ) );
	...
	protected function setup_hooks() {
		add_action( 'ajan_activity_feed_rss_attributes',   array( $this, 'backpat_rss_attributes' ) );
		add_action( 'ajan_activity_feed_channel_elements', array( $this, 'backpat_channel_elements' ) );
		add_action( 'ajan_activity_feed_item_elements',    array( $this, 'backpat_item_elements' ) );
	}
	...
	public function backpat_rss_attributes() {
		do_action( 'ajan_activity_' . $this->id . '_feed' );
	}
	...
	public function backpat_channel_elements() {
		do_action( 'ajan_activity_' . $this->id . '_feed_head' );
	}
	...
	public function backpat_item_elements() {
		...
		do_action( 'ajan_activity_' . $id . '_feed_item' );
	}
	...
	protected function feed_content() {
		...
		switch ( $this->id ) {
			...
			case 'personal' :
				...
				if ( 'activity_comment' == ajan_get_activity_action_name() ) :
			?>
				<strong><?php _e( 'In reply to', 'ajency-activity-and-notifications' ) ?></strong> -
				<?php ajan_activity_parent_content() ?>
			<?php
				endif;
	...
	protected function http_headers() {
		...
		$last_modified      = mysql2date( 'D, d M Y H:i:s O', ajan_activity_get_last_updated(), false );
...
<rss version="2.0"
	...
	<?php do_action( 'ajan_activity_feed_rss_attributes' ); ?>
>
...
<channel>
	...
	<lastBuildDate><?php echo mysql2date( 'D, d M Y H:i:s O', ajan_activity_get_last_updated(), false ); ?></lastBuildDate>
	<generator>http://buddypress.org/?v=<?php ajan_version(); ?></generator>
	...
	<?php do_action( 'ajan_activity_feed_channel_elements' ); ?>

	<?php if ( ajan_has_activities( $this->activity_args ) ) : ?>
		<?php while ( ajan_activities() ) : ajan_the_activity(); ?>
			<item>
				<guid isPermaLink="false"><?php ajan_activity_feed_item_guid(); ?></guid>
				<title><?php echo stripslashes( ajan_get_activity_feed_item_title() ); ?></title>
				<link><?php ajan_activity_thread_permalink() ?></link>
				<pubDate><?php echo mysql2date( 'D, d M Y H:i:s O', ajan_get_activity_feed_item_date(), false ); ?></pubDate>

				<?php if ( ajan_get_activity_feed_item_description() ) : ?>
					<content:encoded><![CDATA[<?php $this->feed_content(); ?>]]></content:encoded>
				<?php endif; ?>

				<?php if ( ajan_activity_can_comment() ) : ?>
					<slash:comments><?php ajan_activity_comment_count(); ?></slash:comments>
				<?php endif; ?>

				<?php do_action( 'ajan_activity_feed_item_elements' ); ?>
			</item>
		<?php endwhile; ?>
