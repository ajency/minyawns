...
/**
 * ActivityNotifications User Query class.
 *
 * Used for querying users in a ActivityNotifications context, in situations where
 ...
 *     @type string $count_total Determines how AJAN_User_Query will do a count
...
class AJAN_User_Query {
	 ...
	 * @since ActivityNotifications (1.7.0)
	 *
	 * @param string|array $query See {@link AJAN_User_Query}.
	 */
	public function __construct( $query = null ) {
		...
		if ( ! empty( $this->query_vars_raw ) ) {
			...
			do_action_ref_array( 'ajan_pre_user_query_construct', array( &$this ) );
		...
		// Get ActivityNotifications specific user data
	 ...
	 *
	 * When extending AJAN_User_Query, you may need to use some of its
	 ...
	 * AJAN_User_Query::__construct() contains a fair amount of logic that
	 ...
	 * {@link AJAN_Group_Member_Query::setup_hooks()} for an example.
	 *
	 * @since ActivityNotifications (1.8.0)
	 */
	...
	public function prepare_user_ids_query() {
		...
		switch ( $type ) {
			...
			// Filter 'ajan_user_query_online_interval' to modify the
			...
			case 'popular' :
				...
				$sql['where'][] = $wpdb->prepare( "u.meta_key = %s", ajan_get_user_meta_key( 'total_friend_count' ) );
			...
			case 'alphabetical' :
				...
				//
				// @todo remove need for ajan_is_active() check
				if ( ! ajan_disable_profile_sync() || ! ajan_is_active( 'xprofile' ) ) {
					$this->uid_name = 'ID';
				...
				// the xprofile table
					...
					$this->uid_name = 'user_id';
					$sql['select']  = "SELECT u.{$this->uid_name} as id FROM {$ajan->profile->table_name_data} u";
					$sql['where'][] = $wpdb->prepare( "u.field_id = %d", ajan_xprofile_fullname_field_id() );
				...
				$sql['where'][] = "u.{$this->uid_name} IN ( SELECT ID FROM {$wpdb->users} WHERE " . ajan_core_get_status_sql( '' ) . " )";
		...
		// @todo remove need for ajan_is_active() check
		if ( ! empty( $user_id ) && ajan_is_active( 'friends' ) ) {
		...
		// xprofile field matches happen in ajan_xprofile_ajan_user_query_search()
		...
		$sql = apply_filters_ref_array( 'ajan_user_query_uid_clauses', array( $sql, &$this ) );
		...
		do_action_ref_array( 'ajan_pre_user_query', array( &$this ) );
	}
	...
	public function do_user_ids_query() {
		...
		if ( 'sql_calc_found_rows' == $this->query_vars['count_total'] ) {
			$this->total_users = $wpdb->get_var( apply_filters( 'ajan_found_user_query', "SELECT FOUND_ROWS()", $this ) );
			...
			$this->total_users = $wpdb->get_var( apply_filters( 'ajan_found_user_query', "{$count_select} {$this->uid_clauses['where']}", $this ) );
		}
	}
	...
	public function do_wp_user_query() {
		...
		$wp_user_query = new WP_User_Query( apply_filters( 'ajan_wp_user_query_args', array(
		...
		foreach ( $wp_user_query->results as $u ) {
			wp_cache_set( 'ajan_core_userdata_' . $u->ID, $u, 'bp' );
		}
	 ...
	 * ids passed to the class constructor. See {@link AJAN_Group_Member_Query}
	 * for an example.
	 *
	 * @since ActivityNotifications (1.8.0)
	 *
	 ...
	 *
	 * Different components will hook into the 'ajan_user_query_populate_extras'
	...
	public function populate_extras() {
		 ...
		 *
		 * Two ActivityNotifications components currently do this:
		 * - XProfile: To override display names
		 * - Friends:  To set whether or not a user is the current users friend
		 *
		 * @see ajan_xprofile_filter_user_query_populate_extras()
		 * @see ajan_friends_filter_user_query_populate_extras()
		 */
		do_action_ref_array( 'ajan_user_query_populate_extras', array( $this, $user_ids_sql ) );

		// Fetch last_active data from the activity table
		$last_activities = AJAN_Core_User::get_last_activity( $this->user_ids );
		...
		// - friend count
		// - latest update
		$total_friend_count_key = ajan_get_user_meta_key( 'total_friend_count' );
		$ajan_latest_update_key   = ajan_get_user_meta_key( 'ajan_latest_update'   );
		...
		$user_metas = $wpdb->get_results( $wpdb->prepare( "SELECT user_id, meta_key, meta_value FROM {$wpdb->usermeta} WHERE meta_key IN (%s,%s) AND user_id IN ({$user_ids_sql})", $total_friend_count_key, $ajan_latest_update_key ) );
...
/**
 * Fetch data about a ActivityNotifications user.
 *
 * AJAN_Core_User class can be used by any component. It will fetch useful
 * details for any user when provided with a user_id.
 *
 * Example:
 *    $user = new AJAN_Core_User( $user_id );
 *    $user_avatar = $user->avatar;
 *	  $user_email = $user->email;
 *    $user_status = $user->status;
 *    etc.
 */
class AJAN_Core_User {
	 ...
	 *
	 * @uses ajan_core_get_userurl() Returns the URL with no HTML markup for
	 *       a user based on their user id.
	 * @uses ajan_core_get_userlink() Returns a HTML formatted link for a
	 *       user with the user's full name as the link text.
	 * @uses ajan_core_get_user_email() Returns the email address for the
	 *       user based on user ID.
	 * @uses ajan_get_user_meta() BP function returns the value of passed
	 *       usermeta name from usermeta table.
	 * @uses ajan_core_fetch_avatar() Returns HTML formatted avatar for a user
	 * @uses ajan_profile_last_updated_date() Returns the last updated date
	 *       for a user.
	 */
	public function populate() {
		...
		if ( !empty( $this->profile_data ) ) {
			$full_name_field_name = ajan_xprofile_fullname_field_name();

			$this->user_url  = ajan_core_get_user_domain( $this->id, $this->profile_data['user_nicename'], $this->profile_data['user_login'] );
			...
			$this->user_url  = ajan_core_get_user_domain( $this->id );
			$this->user_link = ajan_core_get_userlink( $this->id );
			$this->fullname  = esc_attr( ajan_core_get_user_displayname( $this->id ) );
			$this->email     = esc_attr( ajan_core_get_user_email( $this->id ) );
		}

		// Cache a few things that are fetched often
		wp_cache_set( 'ajan_user_fullname_' . $this->id, $this->fullname, 'bp' );
		wp_cache_set( 'ajan_user_email_' . $this->id, $this->email, 'bp' );
		wp_cache_set( 'ajan_user_url_' . $this->id, $this->user_url, 'bp' );

		$this->avatar       = ajan_core_fetch_avatar( array( 'item_id' => $this->id, 'type' => 'full', 'alt' => sprintf( __( 'Avatar of %s', 'ajency-activity-and-notifications' ), $this->fullname ) ) );
		$this->avatar_thumb = ajan_core_fetch_avatar( array( 'item_id' => $this->id, 'type' => 'thumb', 'alt' => sprintf( __( 'Avatar of %s', 'ajency-activity-and-notifications' ), $this->fullname ) ) );
		$this->avatar_mini  = ajan_core_fetch_avatar( array( 'item_id' => $this->id, 'type' => 'thumb', 'alt' => sprintf( __( 'Avatar of %s', 'ajency-activity-and-notifications' ), $this->fullname ), 'width' => 30, 'height' => 30 ) );
		$this->last_active  = ajan_core_get_last_activity( ajan_get_user_last_activity( $this->id ), __( 'active %s', 'ajency-activity-and-notifications' ) );
	}
	...
	public function populate_extras() {
		...
		if ( ajan_is_active( 'friends' ) ) {
			$this->total_friends = AJAN_Friends_Friendship::total_friend_count( $this->id );
		}

		if ( ajan_is_active( 'groups' ) ) {
			$this->total_groups = AJAN_Groups_Member::total_group_count( $this->id );
			$this->total_groups = sprintf( _n( '%d group', '%d groups', $this->total_groups, 'ajency-activity-and-notifications' ), $this->total_groups );
		}
	}
	 ...
	 *
	 * @see AJAN_XProfile_ProfileData::get_all_for_user() for description of
	 *      return value.
	 *
	 * @return array See {@link AJAN_XProfile_Profile_Data::get_all_for_user()}.
	 */
	public function get_profile_data() {
		return AJAN_XProfile_ProfileData::get_all_for_user( $this->id );
	}
	 ...
	 *
	 * Since ActivityNotifications 1.7, use {@link AJAN_User_Query} instead.
	 *
	 * @deprecated 1.7.0 Use {@link AJAN_User_Query}.
	 *
	 * @see AJAN_User_Query for a description of parameters, most of which
	 *      are used there in the same way.
	 *
	 * @param string $type See {@link AJAN_User_Query}.
	 * @param int $limit See {@link AJAN_User_Query}. Default: 0.
	 * @param int $page See {@link AJAN_User_Query}. Default: 1.
	 * @param int $user_id See {@link AJAN_User_Query}. Default: 0.
	 * @param mixed $include See {@link AJAN_User_Query}. Default: false.
	 * @param string|bool $search_terms See {@link AJAN_User_Query}.
	 *        Default: false.
	 * @param bool $populate_extras See {@link AJAN_User_Query}.
	 *        Default: true.
	 * @param mixed $exclude See {@link AJAN_User_Query}. Default: false.
	 * @param string|bool $meta_key See {@link AJAN_User_Query}.
	 *        Default: false.
	 * @param string|bool $meta_value See {@link AJAN_User_Query}.
	 *        Default: false.
	 * @return array {
	...
	public static function get_users( $type, $limit = 0, $page = 1, $user_id = 0, $include = false, $search_terms = false, $populate_extras = true, $exclude = false, $meta_key = false, $meta_value = false ) {
		...
		_deprecated_function( __METHOD__, '1.7', 'AJAN_User_Query' );
		...
		if ( $search_terms && ajan_is_active( 'xprofile' ) ) {
			$sql['join_profiledata_search'] = "LEFT JOIN {$ajan->profile->table_name_data} spd ON u.ID = spd.user_id";
		}

		// Alphabetical sorting is done by the xprofile Full Name field
		if ( 'alphabetical' == $type ) {
			$sql['join_profiledata_alpha'] = "LEFT JOIN {$ajan->profile->table_name_data} pd ON u.ID = pd.user_id";
		}
		...
		$sql['where'] = 'WHERE ' . ajan_core_get_status_sql( 'u.' );

		if ( 'active' == $type || 'online' == $type || 'newest' == $type ) {
			$sql['where_active'] = $wpdb->prepare( "AND um.meta_key = %s", ajan_get_user_meta_key( 'last_activity' ) );
		}

		if ( 'popular' == $type ) {
			$sql['where_popular'] = $wpdb->prepare( "AND um.meta_key = %s", ajan_get_user_meta_key( 'total_friend_count' ) );
		}
		...
		if ( 0 === $include || '0' === $include ) {
			...
			} elseif ( !empty( $user_id ) && ajan_is_active( 'friends' ) ) {
				$friend_ids = friends_get_friend_user_ids( $user_id );
		...
		if ( !empty( $search_terms ) && ajan_is_active( 'xprofile' ) ) {
		...
		$paged_users_sql = apply_filters( 'ajan_core_get_paged_users_sql', join( ' ', (array) $sql ), $sql );
		...
		$total_users_sql = apply_filters( 'ajan_core_get_total_users_sql', join( ' ', (array) $sql ), $sql );
		...
		if ( !empty( $populate_extras ) ) {
			...
			$paged_users = AJAN_Core_User::get_user_extras( $paged_users, $user_ids, $type );
		}
	...
	public static function get_users_by_letter( $letter, $limit = null, $page = 1, $populate_extras = true, $exclude = '' ) {
		...
		$status_sql = ajan_core_get_status_sql( 'u.' );
		...
		$total_users_sql = apply_filters( 'ajan_core_users_by_letter_count_sql', $wpdb->prepare( "SELECT COUNT(DISTINCT u.ID) FROM {$wpdb->users} u LEFT JOIN {$ajan->profile->table_name_data} pd ON u.ID = pd.user_id LEFT JOIN {$ajan->profile->table_name_fields} pf ON pd.field_id = pf.id WHERE {$status_sql} AND pf.name = %s {$exclude_sql} AND pd.value LIKE '{$letter}%%'  ORDER BY pd.value ASC", ajan_xprofile_fullname_field_name() ) );
		$paged_users_sql = apply_filters( 'ajan_core_users_by_letter_sql',       $wpdb->prepare( "SELECT DISTINCT u.ID as id, u.user_registered, u.user_nicename, u.user_login, u.user_email FROM {$wpdb->users} u LEFT JOIN {$ajan->profile->table_name_data} pd ON u.ID = pd.user_id LEFT JOIN {$ajan->profile->table_name_fields} pf ON pd.field_id = pf.id WHERE {$status_sql} AND pf.name = %s {$exclude_sql} AND pd.value LIKE '{$letter}%%' ORDER BY pd.value ASC{$pag_sql}", ajan_xprofile_fullname_field_name() ) );
		...
		// Add additional data to the returned results
		if ( $populate_extras ) {
			$paged_users = AJAN_Core_User::get_user_extras( $paged_users, $user_ids );
		}
	 ...
	 *
	 * Use {@link AJAN_User_Query} with the 'user_ids' param instead.
	 *
	 * @global wpdb $wpdb WordPress database object.
	...
	public static function get_specific_users( $user_ids, $limit = null, $page = 1, $populate_extras = true ) {
		...
		$status_sql = ajan_core_get_status_sql();

		$total_users_sql = apply_filters( 'ajan_core_get_specific_users_count_sql', "SELECT COUNT(ID) FROM {$wpdb->users} WHERE {$status_sql} AND ID IN ({$user_ids})" );
		$paged_users_sql = apply_filters( 'ajan_core_get_specific_users_count_sql', "SELECT ID as id, user_registered, user_nicename, user_login, user_email FROM {$wpdb->users} WHERE {$status_sql} AND ID IN ({$user_ids}) {$pag_sql}" );
	...
	public static function search_users( $search_terms, $limit = null, $page = 1, $populate_extras = true ) {
		...
		$total_users_sql = apply_filters( 'ajan_core_search_users_count_sql', "SELECT COUNT(DISTINCT u.ID) as id FROM {$wpdb->users} u LEFT JOIN {$ajan->profile->table_name_data} pd ON u.ID = pd.user_id WHERE {$status_sql} AND pd.value LIKE '%%{$search_terms}%%' ORDER BY pd.value ASC", $search_terms );
		$paged_users_sql = apply_filters( 'ajan_core_search_users_sql',       "SELECT DISTINCT u.ID as id, u.user_registered, u.user_nicename, u.user_login, u.user_email FROM {$wpdb->users} u LEFT JOIN {$ajan->profile->table_name_data} pd ON u.ID = pd.user_id WHERE {$status_sql} AND pd.value LIKE '%%{$search_terms}%%' ORDER BY pd.value ASC{$pag_sql}", $search_terms, $pag_sql );
	...
	public static function get_user_extras( &$paged_users, &$user_ids, $type = false ) {
		...
		if ( ajan_is_active( 'xprofile' ) && 'alphabetical' != $type ) {
			$names = $wpdb->get_results( $wpdb->prepare( "SELECT pd.user_id as id, pd.value as fullname FROM {$ajan->profile->table_name_fields} pf, {$ajan->profile->table_name_data} pd WHERE pf.id = pd.field_id AND pf.name = %s AND pd.user_id IN ( {$user_ids} )", ajan_xprofile_fullname_field_name() ) );
		...
		if ( 'popular' != $type ) {
			$friend_count = $wpdb->get_results( $wpdb->prepare( "SELECT user_id as id, meta_value as total_friend_count FROM {$wpdb->usermeta} WHERE meta_key = %s AND user_id IN ( {$user_ids} )", ajan_get_user_meta_key( 'total_friend_count' ) ) );
			for ( $i = 0, $count = count( $paged_users ); $i < $count; ++$i ) {
		...
		if ( ajan_is_active( 'friends' ) ) {
			$friend_status = $wpdb->get_results( $wpdb->prepare( "SELECT initiator_user_id, friend_user_id, is_confirmed FROM {$ajan->friends->table_name} WHERE (initiator_user_id = %d AND friend_user_id IN ( {$user_ids} ) ) OR (initiator_user_id IN ( {$user_ids} ) AND friend_user_id = %d )", ajan_loggedin_user_id(), ajan_loggedin_user_id() ) );
			for ( $i = 0, $count = count( $paged_users ); $i < $count; ++$i ) {
		...
		if ( 'active' != $type ) {
			$user_activity = $wpdb->get_results( $wpdb->prepare( "SELECT user_id as id, meta_value as last_activity FROM {$wpdb->usermeta} WHERE meta_key = %s AND user_id IN ( {$user_ids} )", ajan_get_user_meta_key( 'last_activity' ) ) );
			for ( $i = 0, $count = count( $paged_users ); $i < $count; ++$i ) {
		...
		$user_update = $wpdb->get_results( $wpdb->prepare( "SELECT user_id as id, meta_value as latest_update FROM {$wpdb->usermeta} WHERE meta_key = %s AND user_id IN ( {$user_ids} )", ajan_get_user_meta_key( 'ajan_latest_update' ) ) );
		for ( $i = 0, $count = count( $paged_users ); $i < $count; ++$i ) {
	...
	public static function get_last_activity( $user_id ) {
		...
		if ( ! is_array( $user_id ) ) {
			$cache = wp_cache_get( $user_id, 'ajan_last_activity' );
	...
	public static function update_last_activity( $user_id, $time ) {
		...
		// set cache
		wp_cache_set( $user_id, $activity, 'ajan_last_activity' );
	...
	public static function delete_last_activity( $user_id ) {
		...
		$deleted = $wpdb->delete(
			activitynotifications()->members->table_name_last_activity,
		...
		wp_cache_delete( $user_id, 'ajan_last_activity' );
...
/**
 * AJAN_Core_Notification is deprecated.
 *
 * Use AJAN_Notifications_Notification instead.
 *
 * @package ActivityNotifications Core
 * @deprecated since ActivityNotifications (1.9)
 */
class AJAN_Core_Notification {
	...
	public function save() {
		...
		// Update
		if ( !empty( $this->id ) ) {
			$sql = $wpdb->prepare( "UPDATE {$ajan->core->table_name_notifications} SET item_id = %d, secondary_item_id = %d, user_id = %d, component_name = %s, component_action = %d, date_notified = %s, is_new = %d ) WHERE id = %d", $this->item_id, $this->secondary_item_id, $this->user_id, $this->component_name, $this->component_action, $this->date_notified, $this->is_new, $this->id );

		// Save
		} else {
			$sql = $wpdb->prepare( "INSERT INTO {$ajan->core->table_name_notifications} ( item_id, secondary_item_id, user_id, component_name, component_action, date_notified, is_new ) VALUES ( %d, %d, %d, %s, %s, %s, %d )", $this->item_id, $this->secondary_item_id, $this->user_id, $this->component_name, $this->component_action, $this->date_notified, $this->is_new );
		}
	...
	public function populate() {
		...
		if ( $notification = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$ajan->core->table_name_notifications} WHERE id = %d", $this->id ) ) ) {
	...
	public static function check_access( $user_id, $notification_id ) {
		...
		return $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(id) FROM {$ajan->core->table_name_notifications} WHERE id = %d AND user_id = %d", $notification_id, $user_id ) );
	}
	...
	public static function get_all_for_user( $user_id, $status = 'is_new' ) {
		...
		$is_new = ( 'is_new' == $status ) ? ' AND is_new = 1 ' : '';
 		...
 		return $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$ajan->core->table_name_notifications} WHERE user_id = %d {$is_new}", $user_id ) );
	}
	...
	public static function delete_for_user_by_type( $user_id, $component_name, $component_action ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$ajan->core->table_name_notifications} WHERE user_id = %d AND component_name = %s AND component_action = %s", $user_id, $component_name, $component_action ) );
	}
	...
	public static function delete_for_user_by_item_id( $user_id, $item_id, $component_name, $component_action, $secondary_item_id = false ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$ajan->core->table_name_notifications} WHERE user_id = %d AND item_id = %d AND component_name = %s AND component_action = %s{$secondary_item_sql}", $user_id, $item_id, $component_name, $component_action ) );
	}
	...
	public static function delete_from_user_by_type( $user_id, $component_name, $component_action ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$ajan->core->table_name_notifications} WHERE item_id = %d AND component_name = %s AND component_action = %s", $user_id, $component_name, $component_action ) );
	}
	...
	public static function delete_all_by_type( $item_id, $component_name, $component_action, $secondary_item_id ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$ajan->core->table_name_notifications} WHERE item_id = %d AND component_name = %s {$component_action_sql} {$secondary_item_sql}", $item_id, $component_name ) );
	}
}

/**
 * API to create ActivityNotifications buttons.
 *
...
class AJAN_Button {
	 ...
	 * @since ActivityNotifications (1.2.6)
	 *
	 * @param array $args See {@AJAN_Button}.
	...
	public function __construct( $args = '' ) {
		...
		if ( ! ajan_is_active( $this->component ) )
			return false;
		...
		if ( true == $this->block_self && ajan_is_my_profile() )
			return false;

		// No button if you are the current user in a loop
		if ( true === $this->block_self && is_user_logged_in() && ajan_loggedin_user_id() === ajan_get_member_user_id() )
			return false;
		...
		$this->contents = apply_filters( 'ajan_button_' . $this->component . '_' . $this->id, $this->contents, $this, $before, $after );
	}
...
/**
 * Enable oEmbeds in ActivityNotifications contexts.
 *
 * Extends WP_Embed class for use with ActivityNotifications.
 *
 * @since ActivityNotifications (1.5.0)
 *
 * @see WP_Embed
 */
class AJAN_Embed extends WP_Embed {
	...
	public function __construct() {
		...
		if ( ajan_use_embed_in_activity() ) {
			add_filter( 'ajan_get_activity_content_body', array( &$this, 'autoembed' ), 8 );
			add_filter( 'ajan_get_activity_content_body', array( &$this, 'run_shortcode' ), 7 );
		}

		if ( ajan_use_embed_in_activity_replies() ) {
			add_filter( 'ajan_get_activity_content', array( &$this, 'autoembed' ), 8 );
			add_filter( 'ajan_get_activity_content', array( &$this, 'run_shortcode' ), 7 );
		}

		if ( ajan_use_embed_in_forum_posts() ) {
			add_filter( 'ajan_get_the_topic_post_content', array( &$this, 'autoembed' ), 8 );
			add_filter( 'ajan_get_the_topic_post_content', array( &$this, 'run_shortcode' ), 7 );
		}

		if ( ajan_use_embed_in_private_messages() ) {
			add_filter( 'ajan_get_the_thread_message_content', array( &$this, 'autoembed' ), 8 );
			add_filter( 'ajan_get_the_thread_message_content', array( &$this, 'run_shortcode' ), 7 );
		}

		do_action_ref_array( 'ajan_core_setup_oembed', array( &$this ) );
	}
	 ...
	 * enabled, then the URL will be passed to {@link AJAN_Embed::parse_oembed()}
	 * for oEmbed parsing.
	 *
	 * @uses wp_parse_args()
	...
	public function shortcode( $attr, $url = '' ) {
		...
		$attr['discover'] = ( apply_filters( 'ajan_embed_oembed_discover', false ) && current_user_can( 'unfiltered_html' ) );
	 ...
	 *
	 * View an example to add support in {@link ajan_activity_embed()}.
	 *
	...
	public function parse_oembed( $id, $url, $attr, $rawattr ) {
		...
		if ( $id ) {
			...
			$cache = '';
			$cache = apply_filters( 'ajan_embed_get_cache', $cache, $id, $cachekey, $url, $attr, $rawattr );

			// Grab cache and return it if available
			if ( !empty( $cache ) ) {
				return apply_filters( 'ajan_embed_oembed_html', $cache, $url, $attr, $rawattr );
			...
			// If no cache, ping the oEmbed provider and cache the result
				...
				do_action( 'ajan_embed_update_cache', $cache, $cachekey, $id );

				// If there was a result, return it
				if ( $html )
					return apply_filters( 'ajan_embed_oembed_html', $html, $url, $attr, $rawattr );
			}
		}
 ...
 * @since ActivityNotifications (1.7.0)
 */
class AJAN_Walker_Nav_Menu extends Walker_Nav_Menu {
	 ...
	 *
	 * This caused a problem for ActivityNotifications because our primary/secondary
	...
	public function walk( $elements, $max_depth ) {
		...
		foreach ( $elements as $e ) {
			// ActivityNotifications: changed '==' to '==='. This is the only change from version in Walker::walk().
			if ( 0 === $e->$parent_field )
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		// Add HTML classes
		$class_names = join( ' ', apply_filters( 'ajan_nav_menu_css_class', array_filter( $item->class ), $item, $args ) );
		...
		$id = apply_filters( 'ajan_nav_menu_item_id', $id, $item, $args );
		...
		// $output is byref
		$output .= apply_filters( 'ajan_walker_nav_menu_start_el', $item_output, $item, $depth, $args );
	}
}

/**
 * Create a set of ActivityNotifications-specific links for use in the Menus admin UI.
 *
 ...
 */
class AJAN_Walker_Nav_Menu_Checklist extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! in_array( array( 'ajan-menu', 'ajan-'. $item->post_excerpt .'-nav' ), $item->classes ) ) {
			$item->classes[] = 'ajan-menu';
			$item->classes[] = 'ajan-'. $item->post_excerpt .'-nav';
		}
